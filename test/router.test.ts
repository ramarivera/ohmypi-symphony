...
import { AppUserId, OrganizationId, TeamId } from "../src/domain/ids.js";
import { OAuthStateError } from "../src/domain/errors.js";
import { OAuth, Reconciler, WebhookPipeline } from "../src/services/contracts.js";
import { GatewayConfig } from "../src/services/config.js";
import { AdminSessionRepo } from "../src/services/store/repositories.js";
import { oauthCallback, oauthStart, router, webhook } from "../src/http/router.js";

const routes = [...router.routes];

function route(method: string, path: string) {
  return routes.find((candidate) => candidate.method === method && candidate.path === path);
}

function request(method: string, path: string): HttpServerRequest.HttpServerRequest {
  return HttpServerRequest.fromWeb(new Request(`https://gateway.example${path}`, { method }));
}
...
describe("HTTP router parity", () => {
  ...
  it("delegates non-POST webhook methods so the pipeline owns 405 behavior", async () => {
    ...
    const response = await Effect.runPromise(
      webhook.pipe(
        Effect.provideService(HttpServerRequest.HttpServerRequest, request("PUT", "/webhooks/linear")),
  ...
  it("enqueues reconciliation without delaying a successful webhook response", async () => {
    ...
    const pipeline: WebhookPipeline = {
      _tag: "WebhookPipeline",
      handle: (_incoming: Request) => Effect.succeed(new Response(null, { status: 204 })),
    };
    const reconciler: Reconciler = {
      _tag: "Reconciler",
      tick: () => Effect.void,
    ...
    const response = await Effect.runPromise(
      webhook.pipe(
        Effect.provideService(HttpServerRequest.HttpServerRequest, request("POST", "/webhooks/linear")),
        Effect.provideService(WebhookPipeline, pipeline),
        Effect.provideService(Reconciler, reconciler),
      ),
    );
  ...
  it("adds legacy security headers and structured lifecycle logging to OAuth start", async () => {
    ...
    expect(webResponse.headers.get("location")).toBe("https://linear.example/authorize");
    ...
    expect(webResponse.headers.get("referrer-policy")).toBe("strict-origin-when-cross-origin");
    expect(webResponse.headers.get("cache-control")).toBe("no-store");
    expect(logs.some((message) => message.startsWith("oauth.started"))).toBe(true);
  });
  ...
  it("adds security headers and logs OAuth callback completion", async () => {
    ...
    const oauth: OAuth = {
      ...
      completeAuthorization: (_url: URL) =>
        Effect.succeed({
          organizationId: Schema.decodeUnknownSync(OrganizationId)("11111111-1111-4111-8111-111111111111"),
          appUserId: Schema.decodeUnknownSync(AppUserId)("22222222-2222-4222-8222-222222222222"),
          accessToken: "access-token",
          refreshToken: "refresh-token",
          expiresAt: 1_000,
          scopes: [],
          revokedAt: Option.none(),
          accessibleTeamIds: Option.none<ReadonlyArray<Schema.Schema.Type<typeof TeamId>>>(),
    ...
    expect(webResponse.headers.get("referrer-policy")).toBe("strict-origin-when-cross-origin");
    expect(webResponse.headers.get("cache-control")).toBe("no-store");
    expect(webResponse.headers.get("set-cookie")).toContain("omp_gateway_admin=");
    expect(logs.some((message) => message.startsWith("oauth.completed"))).toBe(true);
  });
  it("logs OAuth callback failures without erasing the failure", async () => {
    ...
    const result = await Effect.runPromiseExit(
      oauthCallback.pipe(
        Effect.provideService(HttpServerRequest.HttpServerRequest, request("GET", "/oauth/callback")),
        Effect.provideService(OAuth, oauth),
    ...
    expect(logs.some((message) => message.startsWith("oauth.failed"))).toBe(true);
  });
});
