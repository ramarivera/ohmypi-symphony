...
import { AppUserId, DeliveryId, IssueId, OrganizationId, SessionId } from "../src/domain/ids.js";
  ...
  it.effect("rejects invalid public URLs", () =>
    Effect.gen(function* () {
      const layer = GatewayConfig.Default.pipe(
        Layer.provide(Layer.setConfigProvider(ConfigProvider.fromMap(new Map([...configValues, ["PUBLIC_URL", "http://example.com"]])))),
      );
      const result = yield* Effect.either(Effect.gen(function* () { return yield* GatewayConfig; }).pipe(Effect.provide(layer)));
      expect(result._tag).toBe("Left");
    }),
  );

  it.effect.prop("branded identifier schemas decode arbitrary UUID strings", { value: Schema.UUID }, ({ value }) =>
    Effect.gen(function* () {
      for (const id of [SessionId, DeliveryId, OrganizationId, IssueId, AppUserId]) {
        expect(yield* Schema.decodeUnknown(id)(value)).toBe(value);
      }
    }),
  );

  it.prop("logger redaction removes configured secret values", { secret: Schema.String.pipe(Schema.minLength(1)) }, ({ secret }) => {
    let line = "";
    const logger = pino({ redact: { paths: redactionPaths, censor: "[REDACTED]" } }, { write: (chunk) => { line += String(chunk); } });
    logger.info({ nested: { token: secret }, clientSecret: secret }, "redaction-check");
    const fields = Schema.decodeUnknownSync(Schema.Struct({
