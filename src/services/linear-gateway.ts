...
import type { ActivityId, SourceKey } from "../domain/ids.js";
import { AgentActivitySignal, LinearClient } from "@linear/sdk";
import { Clock, Deferred, Effect, Option, Ref, Redacted, Schema } from "effect";
import {
  DatabaseError,
...
import type { ActivityType, AgentRun, Installation, ProjectionJob, RunEvent, RunInput } from "../domain/models.js";
...
export class LinearGateway extends Effect.Service<LinearGateway>()("LinearGateway", {
  ...
  effect: Effect.gen(function* () {
    ...
    const refreshing = yield* Ref.make(new Map<string, Deferred.Deferred<Installation, TokenRefreshError>>());
    const queues = yield* Ref.make(new Map<string, Deferred.Deferred<void, never>>());
        ...
        yield* installationRepo.put(updated).pipe(
          Effect.catchTags({
            "@Gateway/DatabaseError": (error: DatabaseError) =>
              Effect.fail(new TokenRefreshError({
    ...
    const ensureToken = Effect.fn("LinearGateway.ensureToken")(
      function* (organizationId: string) {
        ...
        const orgId = yield* Schema.decodeUnknown(OrganizationId)(organizationId).pipe(
          Effect.mapError(
            (error) =>
              new TokenRefreshError({
                organizationId,
        ...
        const option = yield* installationRepo.get(orgId).pipe(
          Effect.catchTags({
            "@Gateway/DatabaseError": (error: DatabaseError) =>
              Effect.fail(new TokenRefreshError({ organizationId, message: error.message })),
          ...
          if (error._tag === "@Gateway/LinearRateLimitError") {
            return Option.some(
              Effect.gen(function* () {
                ...
                return yield* withRateLimitRetry(effect);
              }),
            );
          }
          return Option.none();
    ...
    const createActivity = Effect.fn("LinearGateway.createActivity")(
      function* (input: { readonly sessionId: string; readonly content: unknown }) {
        const sessionId = yield* Schema.decodeUnknown(SessionId)(input.sessionId).pipe(
          Effect.mapError(
            (error) =>
              new LinearApiError({
        ...
        if (!isRecord(input.content)) {
          ...
          return yield* Effect.fail(
            new LinearApiError({ operation: "createActivity", message: "Activity content is not a record" }),
          );
        }
        const runOption = yield* runRepo.get(sessionId).pipe(
          Effect.catchTags({
            "@Gateway/DatabaseError": (error: DatabaseError) =>
              Effect.fail(new LinearApiError({ operation: "createActivity", message: error.message })),
            "@Gateway/RowDecodeError": (error: RowDecodeError) =>
              Effect.fail(new LinearApiError({ operation: "createActivity", message: error.message })),
          }),
        );
        const run = yield* Option.match(runOption, {
          onNone: () =>
            Effect.fail(new LinearApiError({ operation: "createActivity", message: `Unknown run ${sessionId}` })),
        ...
        yield* Effect.logInfo("linear.createActivity", { sessionId, activityId: id });
        return id;
      },
    );
    ...
    const updateSession = Effect.fn("LinearGateway.updateSession")(
      function* (input: {
        ...
        const runOption = yield* runRepo.get(sessionId).pipe(
          Effect.catchTags({
            "@Gateway/DatabaseError": (error: DatabaseError) =>
              Effect.fail(new LinearApiError({ operation: "updateSession", message: error.message })),
            "@Gateway/RowDecodeError": (error: RowDecodeError) =>
              Effect.fail(new LinearApiError({ operation: "updateSession", message: error.message })),
          }),
        );
        const run = yield* Option.match(runOption, {
          onNone: () =>
            Effect.fail(new LinearApiError({ operation: "updateSession", message: `Unknown run ${sessionId}` })),
          onSome: Effect.succeed,
        });
        yield* Effect.annotateCurrentSpan({ "linear.organizationId": run.organizationId });
        ...
        yield* withOrgQueue(
          ...
          Effect.gen(function* () {
            ...
            return yield* withRateLimitRetry(
              Effect.tryPromise({
                try: async () => {
                  const payload = await client.updateAgentSession(sessionId, updateInput);
