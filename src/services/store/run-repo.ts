...
import type { IssueId, OrganizationId, SessionId, SourceKey, TeamId, WorkspaceId } from "../../domain/ids.js";
import { AgentRun, RunState } from "../../domain/models.js";
...
export class RunRepo extends Effect.Service<RunRepo>()(
  "RunRepo",
  {
    ...
    effect: Effect.gen(function* () {
      ...
      const get = Effect.fn("RunRepo.get")(
        function* (sessionId: SessionId,): Effect.fn.Return<$$$> { yield* Effect.annotateCurrentSpan("sessionId", sessionId);
      ...
      const create = Effect.fn("RunRepo.create")(
        function* (input: {
          ...
          readonly now?: number;
        }): Effect.fn.Return<$$$> { yield* Effect.annotateCurrentSpan("sessionId", input.sessionId);
        ...
        return yield* Option.match(run, {
          onNone: () => Effect.die(new Error(`Failed to create run ${input.sessionId}`)),
          onSome: Effect.succeed,
        }); },
      );

      const update = Effect.fn("RunRepo.update")(
        ...
        },): Effect.fn.Return<$$$> { yield* Effect.annotateCurrentSpan("sessionId", sessionId);
        const current = yield* get(sessionId);
        if (Option.isNone(current)) {
          return yield* Effect.die(new Error(`Unknown run ${sessionId}`));
        }
        ...
        if (
          ...
          return yield* Effect.die(new Error("Terminal run state is immutable"));
        }
        ...
        if (state !== run.state) {
          const sourceKey = `state:${sessionId}:${attempt}:${state}` as SourceKey;
      ...
      const listRunnable = Effect.fn("RunRepo.listRunnable")(
        function* (now: number,): Effect.fn.Return<$$$> { const rows = yield* tryDb(
          () =>
            db
      ...
      const listCancellationPending = Effect.fn("RunRepo.listCancellationPending")(
        function* (): Effect.fn.Return<$$$> { const rows = yield* tryDb(
          () =>
            db
              .query<AgentRunRow, []>(
      ...
      const claimRun = Effect.fn("RunRepo.claimRun")(
        function* (sessionId: SessionId,
        owner: string,
        leaseDurationMs: number,
        now?: number,): Effect.fn.Return<$$$> { yield* Effect.annotateCurrentSpan("sessionId", sessionId);
      ...
      const renewLease = Effect.fn("RunRepo.renewLease")(
        function* (sessionId: SessionId,
        owner: string,
        leaseDurationMs: number,
        now?: number,): Effect.fn.Return<$$$> { yield* Effect.annotateCurrentSpan("sessionId", sessionId);
      ...
      const releaseLease = Effect.fn("RunRepo.releaseLease")(
        function* (sessionId: SessionId,
        owner: string,): Effect.fn.Return<$$$> { yield* Effect.annotateCurrentSpan("sessionId", sessionId);
        const now = yield* Clock.currentTimeMillis;
        return yield* tryDb(
          () =>
            db
      ...
      const recoverInterruptedRuns = Effect.fn("RunRepo.recoverInterruptedRuns")(
        function* (now?: number): Effect.fn.Return<$$$> { const at = now ?? (yield* Clock.currentTimeMillis);
        const result = yield* tryDb(
          () =>
            db
              .query(`
