...
import type { Installation } from "../../domain/models.js";
import {
  DatabaseError,
  RowDecodeError,
  TokenCipherError,
} from "../../domain/errors.js";
import { SqliteClient, tryDb, decodeRow, transact } from "./sqlite-client.js";
...
const rowToInstallation = (
  row: InstallationRow,
): Effect.Effect<Installation, TokenCipherError | RowDecodeError> =>
  Effect.gen(function* () {
    ...
    const accessibleTeamIds = yield* (row.accessible_team_ids_json === null
      ? Effect.succeed(Option.none<ReadonlyArray<TeamId>>())
      : Effect.gen(function* () {
          const parsed = yield* parseStringArray(
    ...
    const canAccessAllPublicTeams =
      row.can_access_all_public_teams === null
        ? Option.none<boolean>()
        : Option.some(row.can_access_all_public_teams !== 0);
    return {
      ...
      scopes,
      revokedAt:
        row.revoked_at === null ? Option.none<number>() : Option.some(row.revoked_at),
      accessibleTeamIds,
  ...
  "InstallationRepo",
  {
    ...
    effect: Effect.gen(function* () {
      ...
      const put = Effect.fn("InstallationRepo.put")(
        ...
        > {
          yield* Effect.annotateCurrentSpan("organizationId", record.organizationId);
          const [accessToken, refreshToken] = yield* Effect.all(
      ...
      const get = Effect.fn("InstallationRepo.get")(
        function* (
          ...
          DatabaseError | TokenCipherError | RowDecodeError
        > {
          ...
          const installation = yield* rowToInstallation(decoded);
      ...
      const createOAuthState = Effect.fn("InstallationRepo.createOAuthState")(
        function* (
          ...
          return yield* tryDb(
            ...
            "InstallationRepo.createOAuthState",
          );
        },
      );

      const consumeOAuthState = Effect.fn("InstallationRepo.consumeOAuthState")(
        function* (hash: string, now: number): Effect.Effect<boolean, DatabaseError> {
          yield* Effect.annotateCurrentSpan("hash", hash);
