...
import {
  type Installation,
  Installation as InstallationSchema,
...
const rowToInstallation = (
  ...
  Effect.gen(function* () {
    const [accessToken, refreshToken] = yield* Effect.all(
      [
        tokenCrypto.decrypt(row.access_token),
        tokenCrypto.decrypt(row.refresh_token),
      ],
      { concurrency: 2 },
    );
    const scopes = yield* parseStringArray(
      row.scopes_json,
      "installation scopes",
    );
  ...
  "InstallationRepo",
  {
    ...
    effect: Effect.gen(function* () {
      ...
      const put = Effect.fn("InstallationRepo.put")(function* (
        ...
        const [accessToken, refreshToken] = yield* Effect.all(
          [
            tokenCrypto.encrypt(record.accessToken),
            tokenCrypto.encrypt(record.refreshToken),
          ],
          { concurrency: 2 },
        );
      ...
      > {
        ...
        const decoded = yield* decodeRow(InstallationRow, row, "Installation");
      ...
      const revoke = Effect.fn("InstallationRepo.revoke")(function* (
        ...
        yield* tryDb(() => {
          db.query(
            "UPDATE installation SET revoked_at = ? WHERE organization_id = ?",
