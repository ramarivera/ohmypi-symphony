    ...
    effect: Effect.gen(function* () {
      ...
      const enqueue = Effect.fn("ProjectionRepo.enqueue")(
        function* (input: {
          ...
          const tx = Effect.gen(function* () {
            ...
            if ((reserved as { changes: number }).changes === 1) {
              yield* tryDb(
                () =>
                  db
                    .query(`
            ...
            if (existing === null) {
              return yield* Effect.die(
                new Error(`Projection ${input.sourceKey} reservation disappeared`),
              );
            }
      ...
      const claimProjection = Effect.fn("ProjectionRepo.claimProjection")(
        function* (
          ...
          now?: number,
        ): Effect.Effect<Option.Option<ProjectionJob>, DatabaseError | RowDecodeError> {
          ...
          const tx = Effect.gen(function* () {
            ...
            if (row === null) {
              return yield* Effect.die(
                new Error(`Projection ${sourceKey} disappeared after claim`),
              );
            }
      ...
      const listDueProjectionKeys = Effect.fn(
        ...
        function* (
          now?: number,
          limit = 50,
        ): Effect.Effect<ReadonlyArray<SourceKey>, DatabaseError | RowDecodeError> {
          const at = now ?? (yield* Clock.currentTimeMillis);
      ...
      const completeProjection = Effect.fn("ProjectionRepo.completeProjection")(
        function* (
        ...
        ): Effect.Effect<void, DatabaseError | RunLeaseError> {
          yield* Effect.annotateCurrentSpan("sourceKey", sourceKey);
          const now = yield* Clock.currentTimeMillis;
          ...
          const tx = Effect.gen(function* () {
            ...
            if ((completed as { changes: number }).changes !== 1) {
