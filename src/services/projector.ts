...
export class ActivityProjector extends Effect.Service<ActivityProjector>()("ActivityProjector", {
  accessors: true,
  dependencies: [ProjectionRepo.Default, RunRepo.Default, LinearGateway.Default],
      ...
      function* (
        ...
        return yield* Effect.gen(function* () {
          if (job.activityType === "plan" || job.activityType === "externalUrls") {
            ...
            const sessionId = yield* Schema.decodeUnknown(SessionId)(redactedUpdate.sessionId).pipe(
              Effect.mapError(
                (error) =>
                  new LinearApiError({
            ...
            yield* linear.updateSession({ ...redactedUpdate, sessionId });
            yield* projectionRepo.complete(sourceKey, owner, Option.none());
          } else {
            ...
            const sessionId = yield* Schema.decodeUnknown(SessionId)(request.sessionId).pipe(
              Effect.mapError(
                (error) =>
                  new LinearApiError({
