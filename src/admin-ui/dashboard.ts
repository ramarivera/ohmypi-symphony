...
(function () {
  ...
  function serializeMcpForm() {
    ...
    return { id: fields.namedItem("id").value.trim(), name: fields.namedItem("name").value.trim(), transport: fields.namedItem("transport").value, command: fields.namedItem("command").value.trim() || null, url: fields.namedItem("url").value.trim() || null, args: parseMcpArgs(fields.namedItem("args").value), oauthClientId: fields.namedItem("oauthClientId").value.trim() || null, oauthClientSecret: fields.namedItem("oauthClientSecret").value.trim() || null, oauthScope: fields.namedItem("oauthScope").value.trim() || null, env: env, headers: readMcpHeaderRows(), repositoryId: fields.namedItem("repositoryId").value.trim() || null, enabled: !!fields.namedItem("enabled").checked };
  }
